...
- The first scoped rollout showed `/data/internal-event-ledger-r10/ledger.db` remained unavailable after the code stopped requesting exclusive locks. A second rollout against `/data/internal-event-ledger/ledger.db` isolated a startup-time `PRAGMA journal_mode` lock. A third rollout removed that probe; its phase-level logs proved the remaining `SQLITE_BUSY` occurred while creating tables directly on Azure Files. The final runtime builds and closes a complete database on local container storage, then syncs and renames that file onto the mount before opening it. No schema transaction runs on Azure Files. It uses the previously unused stable `/data/internal-event-ledger/ledger-v2.sqlite3`; the generated administrator token stays beside it, and every restart reuses both files. Tests verify the default rollback `DELETE` journal after shutdown. The repair does not delete, rename, or open any earlier ledger.
