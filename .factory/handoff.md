...
- The first scoped rollout showed `/data/internal-event-ledger-r10/ledger.db` remained unavailable after the code stopped requesting exclusive locks. A second rollout against the previously unused `/data/internal-event-ledger/ledger.db` also returned `SQLITE_BUSY`, ruling out a file-specific stale lease and isolating the startup-time `PRAGMA journal_mode` probe. The final runtime removes that lock-taking probe and uses the previously unused stable `/data/internal-event-ledger/events.sqlite3`; its generated administrator token stays beside it. Future revisions and restarts reuse this path. Tests verify the default rollback `DELETE` journal after shutdown. The repair does not delete, rename, or open any earlier ledger.
...
- Earlier abandoned databases, including `internal-event-ledger-r10/ledger.db` and `internal-event-ledger/ledger.db`, are deliberately untouched. Any separate historical-data recovery needs explicit operator authorization.
