...
- Isolation: `POST /api/demo` creates a random in-memory workspace. It expires after 24 hours and never queries the production SQLite tables. Browser state uses only `demo:internal-event-ledger:workspace` so the sample remains readable offline.
