...
test('runtime image carries the supplied build identity and starts with defaults', () => {
  ...
  assert.match(runtime, /^\s*DATABASE_URL="sqlite:\/\/\/data\/internal-event-ledger-r9\/ledger\.db\?mode=rwc" \\/m);
  assert.match(runtime, /^\s*ADMIN_TOKEN_FILE=\/data\/internal-event-ledger-r9\/admin-token \\/m);
...
test('startup uses a fresh data directory, one connection, DELETE journals, and bounded retries', () => {
  ...
  assert.match(dockerfile, /internal-event-ledger-r9\/ledger\.db/);
});
...
test('the SQLite policy is one connection with a rollback DELETE journal', async () => {
  ...
  assert.match(library, /STORAGE_SUBDIRECTORY: &str = "internal-event-ledger-r9"/);
