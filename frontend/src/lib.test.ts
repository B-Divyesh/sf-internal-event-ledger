...
import { escapeHtml, isPublicLegalView, relativeTime, slugify } from './lib';
...
describe('interface helpers', () => {
  ...
  it('keeps privacy and terms public', () => {
    expect(isPublicLegalView('privacy')).toBe(true);
    expect(isPublicLegalView('terms')).toBe(true);
    expect(isPublicLegalView('inbox')).toBe(false);
  });
});
